...
import com.tdunning.math.stats.BatchTDigest2.Centroid;
import com.tdunning.math.stats.BatchTDigest2.Histogram;

public class BatchDigestTest2 {
            ...
            tasks.add(new Callable<String>() {
                ...
                public String call() throws Exception {
                    ...
                    for (int parts : new int[]{2, 5, 10, 20, 50, 100}) {
                        ...
                        BatchTDigest2 dist = factory.create();
                        // dist.recordAllData(); TODO

                        // we accumulate the data into multiple sub-digests
                        List<BatchTDigest2> subs = Lists.newArrayList();
                        for (int i = 0; i < parts; i++) {
                            subs.add(factory.create()); // TODO .recordAllData()
                        }
                        ...
                        BatchTDigest2 dist2 = BatchTDigest2.merge(subs, gen, factory.create());

                        // verify the merged result has the right data
                        List<Double> data3 = data;
    ...
    protected void singleValue(BatchTDigest2 digest) {
    ...
    protected void fewValues(BatchTDigest2 digest) {
    ...
    protected void runTest(DigestFactory factory, AbstractContinousDistribution gen, double sizeGuide, double[] qValues, String tag, boolean recordAllData) {
        ...
        double qz = 0;
        int iz = 0;
        for (com.tdunning.math.stats.BatchTDigest2.Centroid centroid : dist.centroids()) {
    ...
    protected void empty(BatchTDigest2 digest) {
        final double q = RandomUtils.getRandom().nextDouble();
        assertTrue(Double.isNaN(digest.quantile(q)));
    }

    protected void moreThan2BValues(BatchTDigest2 digest) {
    ...
    public void testMergeEmpty() {
        final Random gen0 = RandomUtils.getRandom();
        List<BatchTDigest2> subData = new ArrayList<BatchTDigest2>();
        subData.add(new BatchTDigest2(10));
        BatchTDigest2 foo = new BatchTDigest2(10);
        BatchTDigest2.merge(subData, gen0, foo);
        empty(foo);
    }

    public interface DigestFactory {
        BatchTDigest2 create();
    }

    protected void sorted(BatchTDigest2 digest) {
        Random gen = RandomUtils.getRandom();
    ...
    protected void nan(BatchTDigest2 digest) {
        Random gen = RandomUtils.getRandom();
    ...
    private DigestFactory factory = new DigestFactory() {
        @Override
        public BatchTDigest2 create() {
            return new BatchTDigest2(100);
        }
    };
...
//                            2.6615455373884e+00, 1.5884778179295e+01, 3.3636770117188e+01},
    ...
    public void testRepeatedValues() {
        ...
        BatchTDigest2 dist = new BatchTDigest2((double) 1000);
        List<Double> data = Lists.newArrayList();
    ...
    public void testSerialization() {
        Random gen = RandomUtils.getRandom();
        BatchTDigest2 dist = new BatchTDigest2(100);
        for (int i = 0; i < 100000; i++) {
            double x = gen.nextDouble();
            dist.add(x);
        }
        ...
        buf.flip();
        BatchTDigest2 dist2 = BatchTDigest2.fromBytes(buf);
        ...
        buf.flip();
        dist2 = BatchTDigest2.fromBytes(buf);
        assertEquals(dist.centroids().size(), dist2.centroids().size());
        assertEquals(dist.compression(), dist2.compression(), 0);
        assertEquals(dist.size(), dist2.size());
    ...
    private void compareQD(PrintWriter out, AbstractContinousDistribution gen, String tag, long scale) {
        for (double compression : new double[]{2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000}) {
            ...
            BatchTDigest2 dist = new BatchTDigest2(compression);
            List<Double> data = Lists.newArrayList();
            for (int i = 0; i < 100000; i++) {
                double x = gen.nextDouble();
                dist.add(x);
    ...
    public void testEmpty() {
        empty(new BatchTDigest2(100));
    }

    @Test
    public void testSingleValue() {
        singleValue(new BatchTDigest2(100));
    }

    @Test
    public void testFewValues() {
        final BatchTDigest2 digest = new BatchTDigest2(100);
        fewValues(digest);
    }
    ...
    public void testSorted() {
        final BatchTDigest2 digest = factory.create();
        sorted(digest);
    }
    ...
    public void testHistogramMerge() {
        for (int i = 0; i < 10; ++i) {
            ...
            for (int j = 0; j < len1; ++j) {
                ...
                h1.append(current, 1 + r.nextInt(5));
            }
            ...
            for (int j = 0; j < len2; ++j) {
                current += Double.MIN_VALUE + r.nextDouble();
                h2.append(current, 1 + r.nextInt(5));
            }
    ...
    @Test
    public void testHistogramCompact() {
        for (int i = 0; i < 10; ++i) {
            Random r = new Random(i);
            final int len1 = r.nextInt(10000);
            ...
            for (int j = 0; j < len1; ++j) {
                current += Double.MIN_VALUE + r.nextDouble();
                h.append(current, 1 + r.nextInt(5));
            }
            for (int j = 0; j < 10; ++j) {
                Histogram h2 = new Histogram(false);
