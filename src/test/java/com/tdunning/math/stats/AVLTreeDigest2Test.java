...
//                            2.6615455373884e+00, 1.5884778179295e+01, 3.3636770117188e+01},
    ...
    public void testRepeatedValues() {
        ...
        AVLTreeDigest2 dist = new AVLTreeDigest2((double) 1000);
    ...
    public void testSerialization() {
        Random gen = RandomUtils.getRandom();
        AVLTreeDigest2 dist = new AVLTreeDigest2(100);
        ...
        buf.flip();
        AVLTreeDigest2 dist2 = AVLTreeDigest2.fromBytes(buf);
        ...
        buf.flip();
        dist2 = AVLTreeDigest2.fromBytes(buf);
        assertEquals(dist.centroidCount(), dist2.centroidCount());
        assertEquals(dist.compression(), dist2.compression(), 0);
        assertEquals(dist.size(), dist2.size());
    ...
    private void compareQD(PrintWriter out, AbstractContinousDistribution gen, String tag, long scale) {
        for (double compression : new double[]{2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000}) {
            ...
            AVLTreeDigest2 dist = new AVLTreeDigest2(compression);
            List<Double> data = Lists.newArrayList();
            for (int i = 0; i < 100000; i++) {
                double x = gen.nextDouble();
                dist.add(x);
    ...
    public void testMerge() throws FileNotFoundException, InterruptedException, ExecutionException {
        merge(new DigestFactory<AVLTreeDigest2>() {
            @Override
            public AVLTreeDigest2 create() {
                return new AVLTreeDigest2(50);
            }
        });
    }
    ...
    public void testEmpty() {
        empty(new AVLTreeDigest2(100));
    }

    @Test
    public void testSingleValue() {
        singleValue(new AVLTreeDigest2(100));
    }

    @Test
    public void testFewValues() {
        final TDigest digest = new AVLTreeDigest2(100);
        fewValues(digest);
    }
    ...
    public void testExtremeQuantiles() {
        ...
        AVLTreeDigest2 digest = new AVLTreeDigest2(100);
